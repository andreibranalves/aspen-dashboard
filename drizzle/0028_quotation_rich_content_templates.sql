...
-- Publishes rich-text and configurable-summary visual templates as new immutable v2 versions.
  ...
  <div class="body">
    ...
    <div class="info-grid">
      ...
      <div class="info-block">
        ...
        <div class="section-copy">{{secoes.prazo_producao.value_html}}</div>
      </div>
      {{/if}}
...
</html>$quotation_branded_content_v4$, '08b5e83928c93c4d0887c5da48fe07c18e1f4e1894c7b040eb48e8868b86066c', 2
...
WHERE template."key" = 'branded'
  AND NOT EXISTS (
    ...
    WHERE duplicate."template_id" = template."id"
      AND duplicate."source_hash" = '08b5e83928c93c4d0887c5da48fe07c18e1f4e1894c7b040eb48e8868b86066c'
  )
...
INSERT INTO "quotation_template_versions" ("id", "template_id", "version", "source", "source_hash", "contract_version")
SELECT 'f4000000-0000-4000-8000-000000000004'::uuid, template."id", COALESCE(MAX(existing."version"), 0) + 1, $quotation_comparativo_content_v4$<!doctype html>
<html>
<head>
  <meta charset="utf-8">
...
</html>$quotation_comparativo_content_v4$, 'd7d09ab196380d6e0f0f8d7dcf43d53adb7512b6c134190ee0e94ac9af6633a1', 2
FROM "quotation_templates" AS template
LEFT JOIN "quotation_template_versions" AS existing ON existing."template_id" = template."id"
WHERE template."key" = 'comparativo'
  AND NOT EXISTS (
    SELECT 1 FROM "quotation_template_versions" AS duplicate
    WHERE duplicate."template_id" = template."id"
      AND duplicate."source_hash" = 'd7d09ab196380d6e0f0f8d7dcf43d53adb7512b6c134190ee0e94ac9af6633a1'
  )
GROUP BY template."id"
ON CONFLICT DO NOTHING;
--> statement-breakpoint
...
INSERT INTO "quotation_template_versions" ("id", "template_id", "version", "source", "source_hash", "contract_version")
SELECT 'f4000000-0000-4000-8000-000000000005'::uuid, template."id", COALESCE(MAX(existing."version"), 0) + 1, $quotation_simples_content_v4$<!doctype html>
<html lang="pt-BR">
<head>
  <meta charset="utf-8">
  <title>Orçamento {{quote_number}}</title>
...
</html>$quotation_simples_content_v4$, 'c2f3de122dfcb1d91b8bd553da32d8e03c527fd94cac1f5e30aa528f52a84817', 2
FROM "quotation_templates" AS template
LEFT JOIN "quotation_template_versions" AS existing ON existing."template_id" = template."id"
WHERE template."key" = 'simples'
  AND NOT EXISTS (
    SELECT 1 FROM "quotation_template_versions" AS duplicate
    WHERE duplicate."template_id" = template."id"
      AND duplicate."source_hash" = 'c2f3de122dfcb1d91b8bd553da32d8e03c527fd94cac1f5e30aa528f52a84817'
  )
GROUP BY template."id"
ON CONFLICT DO NOTHING;
--> statement-breakpoint
