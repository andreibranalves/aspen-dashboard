...
async function testResponsive(page, label, width) {
  ...
  check(
    `Sem overflow horizontal em ${label}`,
    bodyWidth <= width + 20,
    `body:${bodyWidth} vs vp:${width}`
  );
  ...
  if ((await kanbanArea.count()) > 0) {
    check(
      `Kanban com scroll horizontal em ${label} (7 colunas = esperado)`,
      true,
      'scroll é necessário com 7 colunas'
    );
  }
