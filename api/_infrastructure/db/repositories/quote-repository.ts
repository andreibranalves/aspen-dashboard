...
import { normalizeLeadSource } from '../../../_modules/lead-source.js';
...
export interface QuoteDraftCreateInput {
  ...
  /** Acquisition channel picked by the operator (`LEAD_SOURCE_VALUES`). */
  origem?: unknown;
...
export function createPostgresQuoteDraftRepository(
  ...
  const createDraft = async (input: QuoteDraftCreateInput): Promise<QuoteDraftResult> => {
    ...
    const leadSource = normalizeLeadSource(input.origem);
    ...
    try {
      ...
      return await database.transaction(async (tx) => {
        ...
        await tx.insert(quotations).values({
          ...
          leadSource: sourceQuotation.leadSource,
          status: 'rascunho',
          createdAt,
          updatedAt: createdAt,
        });
