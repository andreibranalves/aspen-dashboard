...
export function createQuotationPdfHandler(
  dependencies: QuotationPdfHandlerDependencies = {},
): LegacyHandler {
  return async function quotationPdfHandler(event: FunctionEvent): Promise<FunctionResult> {
    ...
    try {
      const { buffer, customerName } = await generateQuotationPdf(quotationId, dependencies);
...
export const handler = createQuotationPdfHandler();
