...
function body(event: FunctionEvent): Record<string, unknown> | null {
  try {
...
function errorResponse(error: unknown): FunctionResult {
  if (
    error instanceof QuotationTemplateLibraryInputError ||
    error instanceof QuotationTemplateLibraryNotFoundError ||
    error instanceof QuotationTemplateLibraryConflictError ||
    error instanceof QuotationTemplateLibraryRepositoryError
  )
...
function isValidate(event: FunctionEvent): boolean {
  return (
    new URL(event.url || '/api/quotation-templates', 'http://localhost').pathname.replace(
      /\/$/,
      ''
...
export interface QuotationTemplatesDependencies {
  repository: QuotationTemplateLibraryRepository;
}
export function createQuotationTemplatesHandler(
  dependencies: QuotationTemplatesDependencies = {
    repository: createQuotationTemplateLibraryRepository(),
  }
): LegacyHandler {
  return async (event) => {
    ...
    try {
      if (isValidate(event)) {
        ...
        return json(
          200,
          await dependencies.repository.validate({
      ...
      if (action === 'save_version')
        return json(
          200,
          await dependencies.repository.saveVersion(id, {
            name: input.name == null ? undefined : String(input.name),
            source: String(input.source || ''),
          })
        );
