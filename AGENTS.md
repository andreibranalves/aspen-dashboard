...
- Merge de PR em `master`, que publica em Production, está autorizado quando todos os checks do PR passam, inclusive o Preview do Vercel; o agente pode tirar o PR de rascunho para isso. PR na lane CRITICAL ou com migration continua exigindo autorização explícita: a migration é aplicada à parte (`docs/database-migrations.md`) e precisa chegar à produção antes do código que depende dela.
- Exigem autorização explícita e separada: push direto em `master`, deploy manual, alteração de env operacional, envio externo, efeitos sobre dados ou comunicação reais e aplicar migration em qualquer alvo que não seja PostgreSQL local descartável ou a branch Neon de Preview do próprio PR (`--target preview`).
