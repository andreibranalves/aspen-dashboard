...
test('Vercel and local route maps expose the same 44 route names', () => {
  const maps = ROUTE_MAP_FILES.map((file) => [file, routeNamesFromSource(file)] as const);
  const expected = maps[0][1];

  assert.equal(expected.size, 44, `${maps[0][0]}: expected 44 routes, found ${expected.size}`);
  for (const [file, routes] of maps.slice(1)) {
    assert.equal(routes.size, 44, `${file}: expected 44 routes, found ${routes.size}`);
    assert.equal(
      routes.size,
      expected.size,
