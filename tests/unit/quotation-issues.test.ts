...
test('POST maps fingerprint or price conflict to 409', async () => {
  const response = await createQuotationIssuesHandler({
    issue: async () => { throw new (await import('../../api/_db/quotation-issue-repository.js')).QuotationIssueConflictError('conteúdo diferente'); },
...
test('POST maps PDF failure to safe Portuguese 503', async () => {
  const response = await createQuotationIssuesHandler({
    issue: async () => { throw new (await import('../../api/_db/quotation-issue-repository.js')).QuotationIssueRepositoryError('Não foi possível gerar o PDF do orçamento. Tente novamente.'); },
  })(event('POST', { 'Idempotency-Key': '00000000-0000-4000-8000-000000000001' }, JSON.stringify({ draft: {} })));
