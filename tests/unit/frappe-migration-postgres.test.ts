...
test('PostgreSQL orçamentos importam com revisão, itens, documento histórico e contador', { skip: !TEST_DATABASE_URL }, async () => {
  const client = postgres(TEST_DATABASE_URL!, { max: 1, prepare: false, connect_timeout: 10, idle_timeout: 20, onnotice: () => undefined });
  const db = drizzle(client, { schema });
  const suffix = `${Date.now()}-${Math.random().toString(16).slice(2)}`;
  ...
  const dataset = {
    ...
    quotations: [{
      ...
      net_total: '50.00',
      grand_total: '50.00',
      items: [{ idx: 1, item_code: sku, item_name: 'Produto PG', qty: '10', uom: 'Und', rate: '5.00', price_list_rate: '5.00', amount: '50.00' }],
    }],
  };
  try {
    await migrate(db, { migrationsFolder });
    ...
    const quotationRows = await db.select().from(schema.quotations).where(sql`${schema.quotations.businessNumber} = ${expectedBusinessNumber}`);
    assert.equal(quotationRows.length, 1);
    assert.equal(quotationRows[0].status, 'enviado');
    const revisionRows = await db.select().from(schema.quoteRevisions).where(eq(schema.quoteRevisions.quotationId, quotationRows[0].id));
    ...
    const itemRows = await db.select().from(schema.quoteRevisionItems).where(eq(schema.quoteRevisionItems.revisionId, revisionRows[0].id));
    assert.equal(itemRows.length, 1);
    assert.equal(itemRows[0].quantidade, '10');
    assert.equal(itemRows[0].precoAplicado, '5.00');
    const documentRows = await db.select().from(schema.issuedDocuments).where(eq(schema.issuedDocuments.revisionId, revisionRows[0].id));
    ...
    const sequenceRows = await db.select().from(schema.quoteSequences).where(eq(schema.quoteSequences.year, 2024));
    assert.ok(sequenceRows.length === 1 && Number(sequenceRows[0].lastNumber) >= sequence);
    const lineageRows = await db.select().from(schema.frappeImportLineage).where(eq(schema.frappeImportLineage.sourceId, quotationId));
    ...
    const quotationRowsAfter = await db.select().from(schema.quotations).where(sql`${schema.quotations.businessNumber} = ${expectedBusinessNumber}`);
    assert.equal(quotationRowsAfter.length, 1);
    const itemRowsAfter = await db.select().from(schema.quoteRevisionItems).where(eq(schema.quoteRevisionItems.revisionId, revisionRows[0].id));
    assert.equal(itemRowsAfter.length, 1);
    const documentRowsAfter = await db.select().from(schema.issuedDocuments).where(eq(schema.issuedDocuments.revisionId, revisionRows[0].id));
    assert.equal(documentRowsAfter.length, 1);

    // A changed source quotation is updated in place without duplicating rows.
    const updated = await runFrappeMigration({ mode: 'apply', dataset: {
      ...dataset,
      quotations: [{ ...dataset.quotations[0], status: 'Ordered' }],
    }, repository });
    assert.equal(updated.report.orcamentos.atualizados, 1);
    const quotationRowsUpdated = await db.select().from(schema.quotations).where(sql`${schema.quotations.businessNumber} = ${expectedBusinessNumber}`);
    ...
    await client.unsafe('DELETE FROM quote_sequences WHERE year = $1 AND last_number <= $2', [2024, sequence]);
    await client.unsafe('DELETE FROM frappe_import_lineage WHERE source_id LIKE $1', [`%${suffix}`]);
    await client.unsafe('DELETE FROM quotations WHERE business_number LIKE $1', [`%${expectedBusinessNumber}`]);
