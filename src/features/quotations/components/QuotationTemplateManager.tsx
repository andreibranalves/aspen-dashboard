      ...
      {!listError && (
        <div className="grid gap-5 lg:grid-cols-[minmax(220px,0.7fr)_minmax(0,1.5fr)]">
          ...
          <div className="space-y-4">
            ...
            <label className="block space-y-1.5 text-sm text-fg"><span className="font-medium">Fonte HTML</span><textarea value={source} onChange={(event) => setSource(event.target.value)} disabled={saving} rows={14} className="w-full resize-y rounded-[10px] border border-line bg-surface px-3.5 py-2.5 font-mono text-xs leading-[1.4] text-fg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/15 disabled:cursor-not-allowed disabled:opacity-50" /></label>
