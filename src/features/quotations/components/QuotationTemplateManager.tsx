            ...
            {templates.map((template) => (
              ...
              >
                ...
                <span className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
                  {template.is_default && <span className="tone-info-soft inline-flex items-center px-2 py-0.5 text-[11px] font-medium">Padrão</span>}
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${template.archived ? 'bg-surface-muted text-fg-muted' : 'tone-success-soft'}`}>{template.archived ? 'Arquivado' : 'Ativo'}</span>
                </span>
              </button>
            ))}
          </div>
