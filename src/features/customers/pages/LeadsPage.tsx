      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-1 rounded-full border border-line bg-surface p-1">
          {(['active', 'archived', 'all'] as const).map((value) => (
            <button key={value} type="button" onClick={() => { setStatus(value); setPage(1); void fetchData(search, 1, value, limit); }} className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${status === value ? 'bg-primary text-on-solid' : 'text-fg-muted hover:text-fg'}`}>
